...
class usersop{
	...
	public function place_order_descrip($orderid = NULL, $itemid = NULL, $userid = NULL, $catagory = NULL, $qty = 1){
		...
		if(isset($orderid) && isset($itemid) && isset($userid) && isset($catagory)){
			...
			$arr = array(	'orderid' => $orderid,
							...
							'catagory' => $catagory,
							'quantity' => $qty);
			$tablename = 'orders_descrip';
			$this->_db->insert($tablename, $arr);
			if(!$this->_db->error()){

				//to increment the flag count
				$result = $this->_db->get($catagory,array('itemid','=',$itemid))->first();
				$flag = $result->flag + 1;
				$stock = $result->stock - $qty;
				$this->_db->update($catagory, array('flag' => $flag, 'stock' => $stock), array('itemid', '=', $itemid));
				$this->_error = false;
	...
	public function place_order($userid = NULL, $name = NULL, $address = NULL, $city = NULL,$state = NULL, $pin = NULL, $phone_no = NULL,$amount = NULL){

		if(isset($userid) && isset($name) && isset($address) && isset($state) && isset($pin) && isset($amount) && isset($city) && isset($phone_no)){
			$arr = array(	'userid' => $userid,
							'name' => $name,
							'address' => $address,
							'city' => $city,
	...
	public function cancel_order($orderid = NULL){
		...
		if(isset($orderid)){
			...
			$this->_db->insert('cancel_orders', $order[0]);
			$orders_detail = $this->_db->get('orders_descrip', array('orderid', '=', $orderid));
			$orders_detail_assoc = $orders_detail->assocresults();
			$count = $this->_db->count();
			$orders_detail_objs = $orders_detail->results();
			for($i = 0; $i<$count; $i++){

				$this->_db->insert('cancel_orders_descrip', $orders_detail_assoc[$i]);
				$orders_detail_obj = $orders_detail_objs[$i];
				$item_stock = $this->_db->get($orders_detail_obj->catagory, array('itemid', '=', $orders_detail_obj->itemid))->first()->stock; 
				$stock = $item_stock + $orders_detail_obj->quantity;
			...
			$this->_db->delete('orders_descrip', array('orderid', '=', $orderid));
	...
	public function recently_viewed($userid = NULL, $itemid = NULL, $catagory = NULL){
		...
		if(isset($itemid) && isset($userid) && isset($catagory)){
				...
				if($this->_db->query("SELECT * FROM recently_viewed WHERE userid = ? AND itemid = ?", array($userid, $itemid), 'SELECT *')->count() != 0){

					return $this;

				}
				$tablename = 'recently_viewed';
				$query = "SELECT * FROM recently_viewed WHERE userid = ? ORDER BY viewedid";

				if($this->_db->query($query, array($userid), 'SELECT *')->count() >= 10){
