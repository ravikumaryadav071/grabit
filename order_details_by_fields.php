...
if($user->isLoggedIn()){
	...
	if($user->hasPermission('admin')){
		...
		if(!empty($_POST)){
			...
			if(isset($_POST['search']) && ($_POST['table_name'] == 'orders')){
				...
				if(isset($_POST['from'])){
					...
					if($_POST['from'] == 'orders'){
						...
						if(isset($_POST['cancel'])){
							...
							$res = $orders[$_POST['result_no']];
							$userop->cancel_order($res['orderid']);
					...
					if($_POST['from'] == 'orders_descrip'){
						...
						if(isset($_POST['edit_item'])){
							...
							$db->query("UPDATE {$_POST['from']} SET {$str} WHERE {$where}", $arr1, 'UPDATE');
				...
				for($i=0; $i < $count1; $i++){
					...
					<input type="hidden" name="from" id="from" value="orders">
					...
					$orders_descrip = $admin->get_order_details('orders_descrip', $arr2)->assocresults();
					$count2 = $db->count();
					...
					for($j=0; $j<$count2; $j++){
						...
						<input type="hidden" name="from" id="from" value="orders_descrip">
						</form>
						</fieldset>
						<?php
					}
		...
		<fieldset><legend>Search Orders By Field</legend>
		<?php
		foreach($result as $key=>$value){
